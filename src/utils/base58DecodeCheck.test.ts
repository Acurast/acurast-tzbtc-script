...
describe("base58DecodeCheck", () => {
  it("should decode and validate the input correctly", () => {
    const input = "eFGDJPketnz";
    const expectedPayload = "deadbeef";

    const result = base58DecodeCheck(input);

    expect(
      Buffer.concat([
        new Uint8Array([result.version]),
        result.payload,
      ]).toString("hex")
  ...
  it("should decode and validate the input correctly", () => {
    fixtures.valid.forEach(({ string, payload }) => {
      const result = base58DecodeCheck(string);
  ...
  it("should throw an error for invalid input", () => {
    ...
    expect(() => {
      base58DecodeCheck(input);
    }).toThrow('Non-base58 character "I"');
  });
});
